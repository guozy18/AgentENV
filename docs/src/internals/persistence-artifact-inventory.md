...
| Paused sandbox artifact generation | `<persisted_sandbox_store_path>/artifacts/{sandbox_id}/{uuid}/...` | Firecracker Temporal continuation artifacts generated by backend pause | Durable artifacts for one paused sandbox generation | Allocated before pause, populated by backend, referenced by paused record. Removed during explicit delete or `load_all` orphan cleanup. |
