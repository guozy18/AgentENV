...
func TestHeartbeatRejectsServiceInstanceNotMatchingDiscovery(t *testing.T) {
	...
	}, time.Unix(100, 0)); !errors.Is(err, ErrServiceInstanceMismatch) {
		t.Fatalf("expected discovery service-instance mismatch, got %v", err)
	}
	...
	if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
		...
		t.Fatalf("expected matching discovered service instance to heartbeat, got %v", err)
	}
}

func TestHeartbeatFencesPreviousServiceInstance(t *testing.T) {
	registry := NewAtomicNodeRegistry([]Node{{ID: "node-a", Endpoint: "http://node-a"}}, 30*time.Second)
	now := time.Unix(100, 0)

	for _, serviceInstanceID := range []string{"svc-old", "svc-new"} {
		if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
			NodeId:            "node-a",
			ClusterId:         "cluster-a",
			ServiceInstanceId: serviceInstanceID,
		}, now); err != nil {
			t.Fatalf("heartbeat %s: %v", serviceInstanceID, err)
		}
	}
	...
	}, now.Add(time.Second)); !errors.Is(err, ErrStaleServiceInstance) {
		t.Fatalf("expected stale old heartbeat to be fenced, got %v", err)
	}

	observed, ok := registry.GetObserved("node-a", "cluster-a", now.Add(time.Second))
	if !ok {
		t.Fatal("expected new service instance to remain observed")
	}
	if got := observed.GetServiceInstanceId(); got != "svc-new" {
		t.Fatalf("expected observed service instance svc-new, got %q", got)
	}
	if err := registry.UnregisterObserved("node-a", "svc-old"); !errors.Is(err, ErrServiceInstanceMismatch) {
		t.Fatalf("expected stale unregister mismatch, got %v", err)
	}
	if err := registry.UnregisterObserved("node-a", "svc-new"); err != nil {
		t.Fatalf("unregister current service instance: %v", err)
	}
}

func TestHeartbeatFencingSurvivesDiscoveryRemovalAndReadd(t *testing.T) {
	registry := NewAtomicNodeRegistry([]Node{{ID: "node-a", Endpoint: "http://node-a"}}, 30*time.Second)
	now := time.Unix(100, 0)
	...
	if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
		...
		t.Fatalf("new heartbeat after rediscovery: %v", err)
	}
	if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
		NodeId:            "node-a",
		ClusterId:         "cluster-a",
		ServiceInstanceId: "svc-old",
	}, now.Add(2*time.Second)); !errors.Is(err, ErrStaleServiceInstance) {
		t.Fatalf("expected old instance to remain fenced after rediscovery, got %v", err)
	}
}

func TestHeartbeatDoesNotInheritCPUConfigAcrossServiceInstances(t *testing.T) {
	firstConfig := `{"cpuid_modifiers":[{"leaf":"0x1","subleaf":"0x0","flags":0,"modifiers":[{"register":"eax","bitmap":"0xff"}]}]}`
	registry := NewAtomicNodeRegistry([]Node{{ID: "node-a", Endpoint: "http://node-a"}}, 30*time.Second)
	now := time.Unix(100, 0)
	...
	if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
		...
		ClusterId:         "cluster-a",
		ServiceInstanceId: "svc-old",
		MachineInfo:       &schedulerv1.MachineInfo{CpuConfigJson: firstConfig},
	}, now); err != nil {
		t.Fatalf("old heartbeat: %v", err)
	}
	if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
	...
	if got := observed.GetMachineInfo().GetCpuConfigJson(); got != "" {
		t.Fatalf("expected replacement instance to provide its own CPU config, got inherited %q", got)
	}
}
...
func TestObservedNodeReplacementWaitsForFreshServiceInstanceHeartbeat(t *testing.T) {
	now := time.Unix(100, 0)
	registry := NewAtomicNodeRegistry([]Node{{
		ID:                "node-a",
		Endpoint:          "http://node-a-old",
		ServiceInstanceID: "svc-old",
	}}, 30*time.Second)
	...
	if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
		...
		t.Fatalf("old service instance heartbeat: %v", err)
	}

	registry.Set([]Node{{
		ID:                "node-a",
		Endpoint:          "http://node-a-new",
		ServiceInstanceID: "svc-new",
	}}, nil)

	observed, ok := registry.GetObserved("node-a", "cluster-a", now)
	if !ok {
		t.Fatal("expected replacement node to remain observed")
	}
	if got := observed.GetEndpoint(); got != "http://node-a-new" {
		t.Fatalf("expected replacement endpoint, got %q", got)
	}
	if got := observed.GetSnapshot().GetStatus(); got != schedulerv1.NodeStatus_NODE_STATUS_CONNECTING {
		t.Fatalf("stale service instance status = %v, want CONNECTING", got)
	}
	...
	if _, _, err := registry.Heartbeat(&schedulerv1.HeartbeatRequest{
		...
		t.Fatalf("replacement service instance heartbeat: %v", err)
	}
	observed, ok = registry.GetObserved("node-a", "cluster-a", now.Add(time.Second))
	if !ok {
		t.Fatal("expected replacement heartbeat to remain observed")
	}
	if got := observed.GetSnapshot().GetStatus(); got != schedulerv1.NodeStatus_NODE_STATUS_READY {
		t.Fatalf("fresh service instance status = %v, want READY", got)
	}
}
